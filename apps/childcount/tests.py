...
class TestApp (TestScript):
    fixtures = ('test.json', 'observations.json', 'lab_codes.json', \
                'diagnoses.json', 'diagnoses_categories.json')
    apps = (App,)

    def setUp(self):
        # since the database is not nuked, there's no point in reloading
        # these every time
    ...
    test_00_Join = '''
        ...
        1234567 > join apple smith ken
        1234567 < Username 'ksmith' is already in use. \
            Reply with: JOIN <last> <first> <username>
        ...
        1234567 < Phone 1234567 is already registered to SMITH, Ken. \
            Reply with 'CONFIRM smithk'.
    ...
    test_02_ListCases = '''
        ...
        7654321 > list
        7654321 < +18 MADISON D. F/%dm, +26 MADISON M. F/%d, +34 \
            MADISON H. F/%dm, +42 MADISON W. M/%dm
    ''' % caseAges
    ...
    test_02_ListProviders = '''
        ...
        7654321 > list @
        7654321 < @1 admin, @2 staff, @3 nonstaff, \
            @4 nonactive, @5 ksmith, @6 smithk, @7 jdoe
    '''
    ...
    test_03_ReportCase = '''
        ...
        7654321 < MUAC> SAM Patient requires OTP care. +26 MADISON, \
            M, F/4 (Sally). MUAC 75 mm

        # cm get converted to mm, g to kg, m to cm
        7654321 > muac +26 7.5 2150 1.4 n
        7654321 < MUAC> SAM Patient requires OTP care. +26 MADISON, \
            M, F/4 (Sally). MUAC 75 mm, 2.1 kg, 140 cm

        # complications list
        7654321 > muac +26 75 21 e a d
        7654321 < MUAC> SAM+ Patient requires IMMEDIATE inpatient care. \
            +26 MADISON, M, F/4 (Sally). MUAC 75 mm, 21.0 kg, \
            Appetite Loss, Diarrhea, Edema

        # complications list - weight is optional
        7654321 > muac +26 75 e a d
        7654321 < MUAC> SAM+ Patient requires IMMEDIATE inpatient care. \
            +26 MADISON, M, F/4 (Sally). MUAC 75 mm, \
            Appetite Loss, Diarrhea, Edema
        ...
        7654321 < MUAC> SAM+ Patient requires IMMEDIATE inpatient care. \
            +26 MADISON, M, F/4 (Sally). MUAC 75 mm, Coughing, Fever, Vomiting


        # one last complication test
        7654321 > muac +26 75 21 n u
        7654321 < MUAC> SAM+ Patient requires IMMEDIATE inpatient care. \
            +26 MADISON, M, F/4 (Sally). MUAC 75 mm, 21.0 kg, Unresponsive

        # MAM logic test
        7654321 > muac +26 120 n
        7654321 <  MUAC> MAM Child requires supplemental feeding.. \
            +26 MADISON, M, F/4 (Sally). MUAC 120 mm

        # Healthy logic test
        7654321 > muac +26 125 n
        7654321 <  MUAC> Child is not malnourished. +26 MADISON, \
            M, F/4 (Sally). MUAC 125 mm
    ...
    test_05_Fever = '''
        ...
        7654321 < MUAC> SAM+ Patient requires IMMEDIATE inpatient care. \
            +26 MADISON, M, F/4 (Sally). MUAC 105 mm, \
    ...
    test_06_Lists = '''
        ...
        7654321 <  MUAC> SAM+ Patient requires IMMEDIATE inpatient care. \
            +26 MADISON, M, F/4 (Sally). MUAC 105 mm, \
            Diarrhea, Fever, Vomiting
        # 7654322 < @jdoe reports +26: SAM+, MUAC 105 mm, \
            Diarrhea, Fever, Vomiting
    '''
    ...
    test_07_mrdt_01 = '''
        ...
        7654321 < MRDT> Child +26, MADISON, Molly, F/4 (Sally), None. \
            RDT=N, Bednet=Y, (Edema, Fever). \
            Please refer patient IMMEDIATELY for clinical evaluation
        7654322 < MRDT> Negative MRDT with Fever. +26, MADISON, Molly, \
            F/4 None. Patient requires IMMEDIATE referral. \
            Reported by CHW J DOE @jdoe m:7654321.
        ...
        7654321 > mrdt +26 n n
        7654321 < MRDT> Child +26, MADISON, Molly, F/4 (Sally), None. \
            RDT=N, Bednet=N. Please refer patient \
            IMMEDIATELY for clinical evaluation
        7654322 < MRDT> Negative MRDT with Fever. +26, MADISON, Molly, \
            F/4 None. Patient requires IMMEDIATE referral. \
            Reported by CHW J DOE @jdoe m:7654321.
        ...
        7654321 < MRDT> Child +34, MADISON, Molly, F/12m has MALARIA. \
            Child is less than 3. Please provide 1 tab of Coartem (ACT)\
             twice a day for 3 days
        7654322 < MRDT> Child +34, MADISON, Molly, F/12m (None) has MALARIA.\
             CHW: @jdoe 7654321
        ...
        7654321 < MRDT> Child +34, MADISON, Molly, F/12m has MALARIA. \
            Child is less than 3. Please provide 1 tab of Coartem (ACT) \
            twice a day for 3 days
        7654322 < MRDT> Child +34, MADISON, Molly, F/12m (None) has \
            MALARIA. CHW: @jdoe 7654321

        7654321 > new madison foo f %s emily
        7654321 < New +42: MADISON, Foo F/3 (Emily) Whiskey

        7654321 > mrdt +42 y n cf e
        7654321 < MRDT> Child +42, MADISON, Foo, F/3 has MALARIA and \
            danger signs (Confusion). Refer to clinic immediately \
            after 2 tabs of Coartem is given
        7654322 < MRDT> Child +42, MADISON, Foo, F/3 (None) has MALARIA \
            and danger signs (Confusion). CHW: @jdoe 7654321


        7654321 > mrdt +42 y n e
        7654321 < MRDT> Child +42, MADISON, Foo, F/3 has MALARIA. \
            Child is 3. Please provide 2 tabs of Coartem (ACT) \
            twice a day for 3 days
        7654322 < MRDT> Child +42, MADISON, Foo, F/3 (None) has \
            MALARIA. CHW: @jdoe 7654321
        ...
        7654321 < MRDT> Child +59, MADISON, Sam, F/2m has MALARIA and\
             danger signs (Confusion). Refer to clinic immediately \
            after 1 tab of Coartem is given
        7654322 < MRDT> Child +59, MADISON, Sam, F/2m (None) has MALARIA \
            and danger signs (Confusion). CHW: @jdoe 7654321


        7654321 > mrdt +59 n n cf e
        7654321 < MRDT> Child +59, MADISON, Sam, F/2m (Samantha), None. \
            RDT=N, Bednet=N, (Confusion, Edema). Please refer patient \
            IMMEDIATELY for clinical evaluation
        7654322 < MRDT> Negative MRDT with Fever. +59, MADISON, Sam, \
            F/2m None. Patient requires IMMEDIATE referral. \
            Reported by CHW J DOE @jdoe m:7654321.
    ''' % (date_boundaries()["over 1 year"], \
           date_boundaries()["over 3 years"], \
           date_boundaries()["over 2 months"])
    ...
    test_08_diagnosis = '''
        ...
        7654321 > D +59 Looked ill /HB+10 had -084.9 and -480 \
            and so on /ELISA-
        7654321 < D> +59 S.MADISON C. Malaria, Viral Pneumonia Labs: \
            HB 10, ELISA-
    '''
    ...
    test_09_errors = '''
        7654321 > foo!
        7654321 < Unknown or incorrectly formed command: foo!...\
             Please call 999-9999
    '''
    ...
    test_11_cancel_fails = '''
        ...
        7654321 < MRDT> Child +108, JAMES, Billy, M/22m has MALARIA. \
            Child is less than 3. Please provide 1 tab of Coartem (ACT) \
            twice a day for 3 days
        7654322 < MRDT> Child +108, JAMES, Billy, M/22m (None) has \
            MALARIA. CHW: @jdoe 7654321
