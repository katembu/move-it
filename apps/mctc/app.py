...
class App (rapidsms.app.App):
    ...
    keyword.prefix = ["nouv"]
    #@keyword(r'@(\S*) (\S+) (\S+) ([MF]) ([\d\-]+) (\S+) (\S+)?( \d+)?')
    @keyword(r'(\S+) (\S+) ([MF]) ([\d\-]+) (\S+) (\S+)?( \d+)?')
    @registered
    def new_case_frccsn (self, message, last, first, gender, dob, guardian_last, guardian_first, contact=""):
        """Format: nouv [patient last name] [patient first name] gender[m/f] [dob ddmmyy] [guardian last name] [guardian first name] [contact #]
       Purpose: register a new patient into the system.  Receive patient ID number back"""
